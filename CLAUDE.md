...
- `client_connect_looper` runs in both delivery modes. `evict_superseded_connection` removes a local `Single` entry only if it connected earlier than the claim, so nothing more is pushed to it. It does **not** end the stream: the sender moves to `parked_streams` (keyed by `StreamToken`) and is released when that stream's own `ClientDisconnection` arrives (client half-close, error, or the request timeout).
- Why the stream must stay open: the shipped Android `GRPCService` uses one shared `StreamObserver` for every stream and one `observer` field for sending ACKs. Any end of *any* of its streams (`onCompleted`, or `onError` with any status) sets `observer = null`, so the live stream keeps receiving notifications but stops ACKing until the next restart. Ending an evicted stream from the server (with `UNAVAILABLE`, `ALREADY_EXISTS`, or a clean close) therefore breaks the live stream's ACKs; `ALREADY_EXISTS` additionally makes the app reconnect at once and loop.
...
- Metrics: `client_connect_messages_total{outcome=self|evicted|kept|not_held}` and `client_slot_events_total{event="evicted_by_peer"|"parked_released"}`. Every eviction parks one stream, so streams currently parked on a pod = `evicted_by_peer` − `parked_released`. Nothing is ever pushed to a parked stream.
