...
impl RedisConnectionPool {
    ...
    pub async fn new(conf: RedisSettings) -> Result<Self> {
        let (pool, join_handles) = Self::instantiate(&conf).await?;
    ...
    async fn instantiate(
        conf: &RedisSettings,
    ) -> Result<(fred::pool::RedisPool, Vec<fred::types::ConnectHandle>)> {
        ...
        let pool =
            fred::pool::RedisPool::new(config, None, Some(reconnect_policy), conf.pool_size)?;
